...
import ProctoringPanel from '../../components/exam/proctoring/ProctoringPanel'
import { getDeveloperProctoringPolicy, isViolationEnabled } from '../../config/proctoringPolicy'
...
const ExamPage = () => {
  ...
  const { violationCount, violations } = useSelector((state) => state.proctoring)
  ...
  const [fullscreenActiveState, setFullscreenActiveState] = useState(Boolean(document.fullscreenElement || document.webkitFullscreenElement))
  ...
  const escAttemptAtRef = useRef(0)
  const fullscreenRecoveryRetryTimeoutRef = useRef(null)

  const proctoringPolicy = getDeveloperProctoringPolicy()
  ...
  const requestFullscreenRecovery = useCallback(async () => {
    ...
    try {
      ...
      if (fullscreenRecoveryRetryTimeoutRef.current) {
        clearTimeout(fullscreenRecoveryRetryTimeoutRef.current)
        fullscreenRecoveryRetryTimeoutRef.current = null
      }
      return true
    } catch (err) {
      console.warn('Failed to recover fullscreen mode:', err)

      // Strict mode: no user prompt. Retry recovery silently.
      if (!fullscreenRecoveryRetryTimeoutRef.current) {
        fullscreenRecoveryRetryTimeoutRef.current = setTimeout(() => {
          fullscreenRecoveryRetryTimeoutRef.current = null
          if (!examEndingRef.current && examInProgress && !examSubmitted) {
  ...
  }, [examInProgress, examSubmitted, isFullscreenActive, lockEscapeKeyInFullscreen, requestFullscreenMode])
  ...
  const requestPermissions = useCallback(async () => {
    ...
    try {
      const mediaStream = await navigator.mediaDevices.getUserMedia({
        video: proctoringPolicy.requireCamera,
        audio: proctoringPolicy.requireMicrophone
      })
      if (proctoringPolicy.requireCamera) {
        setCameraPermission(true)
        setCameraPermissionState('granted')
      }
      if (proctoringPolicy.requireMicrophone) {
      ...
      setPermissionError('Required proctoring device permissions were not granted.')
    ...
    proctoringPolicy.requireCamera,
    proctoringPolicy.requireMicrophone,
    refreshPermissionStates
  ])

  // Ask for proctoring permission every time this exam page is opened.
  useEffect(() => {
    setCameraPermission(!proctoringPolicy.requireCamera)
    setMicrophonePermission(!proctoringPolicy.requireMicrophone)
    setCameraPermissionState('prompt')
    setMicrophonePermissionState('prompt')
    setTestStarted(false)
    refreshPermissionStates()
    if (proctoringPolicy.requireCamera || proctoringPolicy.requireMicrophone) {
      requestPermissions()
    }
  }, [
    applicationId,
  ...
  useEffect(() => {
    const timerId = globalThis.setInterval(() => {
      ...
      setRecordingDurationSec(Math.floor((Date.now() - recordingStartTimeRef.current) / 1000))
    }, 1000)
  ...
  useEffect(() => {
    const intervalId = globalThis.setInterval(() => {
      const stream = webcamRef.current?.stream || null
      if (stream && stream !== previewStream) {
  ...
  const handleStartExamClicked = async () => {
    ...
    if (proctoringPolicy.requireScreenShare) {
    ...
    if (proctoringPolicy.requireFullscreen) {
      const enteredFullscreen = await requestFullscreenMode()
      if (!enteredFullscreen) {
        dispatch(startExamFailure('Fullscreen mode is required to start the exam. Please allow fullscreen and try again.'))
        return
      }
      await lockEscapeKeyInFullscreen()
    }
    ...
    const started = await startExam({
      skipScreenShareRequest: true,
      skipFullscreenRequest: proctoringPolicy.requireFullscreen
    })
    setTestStarted(started)
  }

  const startExam = async ({ skipScreenShareRequest = false, skipFullscreenRequest = false } = {}) => {
    ...
    try {
      ...
      if (proctoringPolicy.requireScreenShare && !skipScreenShareRequest) {
      ...
      if (proctoringPolicy.requireFullscreen && !skipFullscreenRequest) {
        const enteredFullscreen = await requestFullscreenMode()
        if (!enteredFullscreen) {
          throw new Error('Fullscreen mode is required to start the exam.')
        }
        await lockEscapeKeyInFullscreen()
  ...
  const startVideoRecording = async () => {
    ...
    if (webcamRef.current?.stream) {
      const activeStream = webcamRef.current.stream
      ...
      if (proctoringPolicy.monitorNoise) {
  ...
  const reportViolationEvent = useCallback((payload) => {
    if (!isViolationEnabled(payload.type)) {
      return
    }

    dispatch(recordViolation(payload))

    if (!proctoringPolicy.trackViolationsToServer) {
      return
    }
  ...
  }, [dispatch, currentSession, mapViolationTypeForApi, proctoringPolicy.trackViolationsToServer])
  ...
  const preventExamInteractionCopy = (e) => {
    ...
    if (!proctoringPolicy.blockCopyActions) return
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      ...
      if (!proctoringPolicy.monitorNavigationLoss) {
        return
      }
      ...
      if (!document.hidden && examInProgress && proctoringPolicy.requireFullscreen) {
        requestFullscreenRecovery()
      }
    }
    ...
    const handleBlur = () => {
      ...
      if (examInProgress && proctoringPolicy.requireFullscreen) {
        setTimeout(() => {
          requestFullscreenRecovery()
        }, 0)
      }
    ...
    const handleKeyDown = (e) => {
      ...
      if (proctoringPolicy.requireFullscreen && e.key === 'Escape') {
        e.preventDefault()
        e.stopPropagation()
        escAttemptAtRef.current = Date.now()
        showBlockedActionWarning('Escape is disabled during the exam.')
        reportViolationEvent({
          type: 'FULLSCREEN_EXIT_ATTEMPT',
          description: 'User attempted to exit fullscreen mode using Escape during exam',
          severity: 'HIGH',
          timestamp: new Date().toISOString()
        })

        // If the browser still exits fullscreen on Escape, recover immediately.
        setTimeout(() => {
          requestFullscreenRecovery()
        }, 0)
        return
      ...
      if (!proctoringPolicy.blockRestrictedShortcuts) return
    ...
    if (proctoringPolicy.blockCopyActions) {
    ...
    examInProgress,
    proctoringPolicy.blockCopyActions,
    proctoringPolicy.blockRestrictedShortcuts,
    proctoringPolicy.monitorNavigationLoss,
    reportViolationEvent,
    requestFullscreenRecovery,
    screenShareActive
  ...
  useEffect(() => {
    ...
    const handleFullscreenChange = () => {
      ...
      setFullscreenActiveState(false)
      ...
      if (!isFullscreenActive()) {
        const recentlyHandledEscape = Date.now() - escAttemptAtRef.current < 1500
        if (recentlyHandledEscape) {
          setTimeout(() => {
            requestFullscreenRecovery()
          }, 0)
          return
    ...
    isFullscreenActive,
    lockEscapeKeyInFullscreen,
    proctoringPolicy.requireFullscreen,
    reportViolationEvent,
    requestFullscreenRecovery
  ])

  // Sync fullscreen state for UI lock overlays and fallback browsers.
  useEffect(() => {
    const syncFullscreenState = () => {
      setFullscreenActiveState(isFullscreenActive())
    }
  ...
  // Keep fullscreen enforced for the whole exam unless submitted or auto-terminated.
  useEffect(() => {
    if (!proctoringPolicy.requireFullscreen || !examInProgress || examSubmitted) {
      return
    }
    ...
    return () => globalThis.clearInterval(intervalId)
  }, [examInProgress, examSubmitted, isFullscreenActive, proctoringPolicy.requireFullscreen, requestFullscreenRecovery])

  // Try fullscreen recovery when focus returns or on user interaction after fullscreen drops.
  useEffect(() => {
    if (!proctoringPolicy.requireFullscreen || !examInProgress || examSubmitted) {
      return
    }
    ...
    const recoverIfNeeded = () => {
      if (!fullscreenEnforcementReadyRef.current || examEndingRef.current) {
        return
      }
      if (!isFullscreenActive()) {
        requestFullscreenRecovery()
    ...
    globalThis.addEventListener('focus', recoverIfNeeded)
    document.addEventListener('visibilitychange', handleVisibilityRecovery)
    document.addEventListener('click', recoverIfNeeded, true)
    document.addEventListener('keydown', recoverIfNeeded, true)
    document.addEventListener('touchstart', recoverIfNeeded, true)

    return () => {
      globalThis.removeEventListener('focus', recoverIfNeeded)
      document.removeEventListener('visibilitychange', handleVisibilityRecovery)
      document.removeEventListener('click', recoverIfNeeded, true)
      document.removeEventListener('keydown', recoverIfNeeded, true)
      document.removeEventListener('touchstart', recoverIfNeeded, true)
    }
  }, [examInProgress, examSubmitted, isFullscreenActive, proctoringPolicy.requireFullscreen, requestFullscreenRecovery])
  ...
  }, [violations, lastViolation, violationCount])
  ...
  // Cleanup streams and monitor timers on unmount
  useEffect(() => {
    return () => {
      if (screenShareCheckIntervalRef.current) {
        clearInterval(screenShareCheckIntervalRef.current)
  ...
  }, [clearScreenShareRetryTimers, unlockKeyboardIfNeeded])
  ...
  const hasRequiredPermissions =
    (!proctoringPolicy.requireCamera || cameraPermission) &&
    (!proctoringPolicy.requireMicrophone || microphonePermission)
          ...
          <Grid item xs={12} md={8}>
            <Paper sx={{ p: 3 }}>
              ...
              <Typography variant="body1" sx={{ mb: 2 }}>
                Grant the required device permissions for the selected proctoring mode to continue.
              </Typography>
              ...
              {isDenied && (
                <Alert severity="warning" sx={{ mb: 2 }}>
                  ...
                  <Typography variant="body2" sx={{ mb: 1 }}>Please allow the required devices in browser site settings for this page.</Typography>
              ...
              <Box sx={{ display: 'flex', gap: 1, mb: 3 }}>
                <Tooltip title="Camera access required">
                  <IconButton size="small" color={!proctoringPolicy.requireCamera || cameraPermission ? 'primary' : 'error'}>
                    {cameraPermission ? <VideocamIcon /> : <VideocamOffIcon />}
                  </IconButton>
                </Tooltip>
                <Tooltip title="Microphone access required">
                  <IconButton size="small" color={!proctoringPolicy.requireMicrophone || microphonePermission ? 'primary' : 'error'}>
              ...
              <Button variant="contained" onClick={requestPermissions} disabled={isRequestingPermissions} fullWidth>
                {isRequestingPermissions ? 'Requesting Permission...' : 'Retry Required Device Access'}
              </Button>
            </Paper>
          </Grid>
          ...
          <Grid item xs={12} md={4}>
            <Paper sx={{ p: 2.5, height: '100%' }}>
              ...
              <Box sx={{ display: 'grid', gap: 1.25 }}>
                ...
                {proctoringPolicy.requireScreenShare && (
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
    ...
    const setupFlow = [
      {
        key: 'permissions',
        label: 'Permissions',
        helper: hasRequiredPermissions ? 'Ready' : 'Pending',
        done: hasRequiredPermissions
      },
      {
        key: 'security',
  ...
  const lastNoiseViolation = [...violations].reverse().find((v) => v.type === 'BACKGROUND_NOISE')
  const lastFaceViolation = [...violations].reverse().find((v) => v.type === 'FACE_MISSING')
  const lastHeadViolation = [...violations].reverse().find((v) => v.type === 'HEAD_MOVEMENT')

  const panelStatus = {
    camera: cameraPermission ? 'connected' : 'blocked',
    microphone: microphonePermission ? 'connected' : 'blocked',
    screenShare: screenShareActive ? 'active' : 'idle',
    fullscreen: fullscreenActiveState ? 'active' : 'inactive',
    recording: mediaRecorderRef.current?.state === 'recording' ? 'active' : 'idle',
    network: networkOnline ? 'online' : 'offline',
    face: lastFaceViolation ? 'missing' : 'detected',
    head: lastHeadViolation ? 'moving' : 'stable',
    audioEnvironment: lastNoiseViolation ? 'noisy' : 'quiet',
    tabSwitchCount: violations.filter((v) => v.type === 'TAB_SWITCH').length
  }

  const panelEvents = violations.map((v, idx) => ({
    id: `${v.type}-${v.timestamp}-${idx}`,
    timestamp: Date.parse(v.timestamp) || Date.now(),
    severity: v.severity || 'MEDIUM',
    message: v.description || v.type
  }))

  const panelSession = currentSession
    ? {
      sessionId: currentSession.examSessionId ? String(currentSession.examSessionId) : 'N/A',
      uploadedChunks: 0,
      pendingChunks: 0,
      failedChunks: 0,
      violationScore: violationCount,
      warningCount: Math.min(violationCount, 2),
      status: examSubmitted ? 'ENDED' : 'ACTIVE'
    }
    : null
  ...
  return (
    <Container maxWidth="xl" sx={{ p: 0, px: { sm: '8px' } }}>
      <Grid container spacing={1}>
        ...
        <Grid item xs={12} md={4} lg={3}>
          <Grid container spacing={0.75} sx={{ position: { md: 'sticky' }, top: { md: 12 } }}>
            ...
            <Grid item xs={12}>
              <Box>
                {/* Hidden webcam feed keeps existing recording pipeline untouched. */}
                ...
                <ProctoringPanel
                  ...
                  runtimeError={examError}
                />
                {violationCount > 0 && (
                  <Alert severity={violationCount >= 3 ? 'error' : 'warning'} sx={{ mt: 1 }}>
      ...
      <Dialog open={showViolationAlert && violationCount < 3 && !showTerminationNotice} onClose={() => setShowViolationAlert(false)} maxWidth="sm" fullWidth>
      ...
      <Dialog
        open={
          ...
          proctoringPolicy.requireFullscreen &&
          fullscreenEnforcementReadyRef.current &&
      ...
      >
        <DialogTitle sx={{ bgcolor: '#fff7ed', color: '#9a3412', fontWeight: 700 }}>
          Fullscreen Required
        </DialogTitle>
        <DialogContent sx={{ pt: 2 }}>
          <Alert severity="warning" sx={{ mb: 2 }}>
          ...
          <Typography variant="body2">
            Return to fullscreen immediately to continue your exam. Violations are still tracked while fullscreen is not active.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={requestFullscreenRecovery} variant="contained" fullWidth>
