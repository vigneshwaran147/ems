...
import { getDeveloperProctoringPolicy, isViolationEnabled, PROCTORING_DEVELOPER_CONFIG } from '../../config/proctoringPolicy'
...
const ExamPage = () => {
  ...
  const startExam = async ({ skipScreenShareRequest = false } = {}) => {
    ...
    try {
      ...
      try {
        if (proctoringPolicy.requireFullscreen && document.documentElement.requestFullscreen) {
          await document.documentElement.requestFullscreen({ navigationUI: 'hide' })
          await lockEscapeKeyInFullscreen()
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e) => {
      if (!examInProgress || !proctoringPolicy.blockRestrictedShortcuts) return
    ...
    return (
      <Container maxWidth="lg" sx={{ py: 3 }}>
        ...
        <Alert severity="info" sx={{ mb: 2 }}>
          Proctoring policy is controlled by developer config.
          Active mode: {PROCTORING_DEVELOPER_CONFIG.mode}
        </Alert>
  ...
  const enabledChecks = {
    camera: proctoringPolicy.requireCamera,
    microphone: proctoringPolicy.requireMicrophone,
    screenShare: proctoringPolicy.requireScreenShare,
    fullscreen: proctoringPolicy.requireFullscreen,
    recording: proctoringPolicy.requireCamera,
    network: true,
    face: proctoringPolicy.requireCamera,
    head: proctoringPolicy.requireCamera,
    audioEnvironment: proctoringPolicy.monitorNoise
  }
